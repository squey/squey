...
{
	const PVRush::PVNraw::nraw_table_line* str_nraw = values.first;
	float* mapped_values = values.second;

	init_from_first(mandatory_param_value(str_nraw->at(0), mapped_values[0]));
	for (PVCol i = 0; i < str_nraw->size(); i++) {
		operator()(mandatory_param_value(str_nraw->at(i), mapped_values[i]));
	}
