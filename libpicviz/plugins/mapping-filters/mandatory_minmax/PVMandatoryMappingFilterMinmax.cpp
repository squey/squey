...
int Picviz::PVMandatoryMappingFilterMinmax::operator()(mandatory_param_list_values const& values)
{
	...
	const PVRush::PVNraw::nraw_table_line* str_values = values.first;
	...
	mandatory_param_value ymin = mandatory_param_value(str_values->at(mmar.get_min_index()), mmar.get_min_value());
	mandatory_param_value ymax = mandatory_param_value(str_values->at(mmar.get_max_index()), mmar.get_max_value());
