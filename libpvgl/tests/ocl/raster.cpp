...
void ocl_raster(const char* kernels_file, const float* yl, const float* yr, const size_t n, unsigned int* img_idxes, const float zoom_x, const float zoom_y)
{
	...
	// build the program
	err = clBuildProgram(program, 0, NULL, "-g -s /home/aguinet/pv/libpvgl/tests/ocl/raster.cl", NULL, NULL);
	verify(err == CL_SUCCESS);

	// create the kernel
	kernel = clCreateKernel(program, "raster", NULL);
	verify(kernel != 0);
	...
	cl_mem mem_yl = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, NLINES_PER_KERNEL*sizeof(float), (void*) yl, NULL);
	cl_mem mem_yr = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, NLINES_PER_KERNEL*sizeof(float), (void*) yr, NULL);
	...
	const unsigned int nlines_per_kernel = NLINES_PER_KERNEL;
	...
	clFinish(cmd_queue);
	BENCH_END(ocl, "ocl raster", lend, 2*sizeof(float), 1, SIZE_GLOBAL_IDX_TABLE);


	// read output image indexes
	err = clEnqueueReadBuffer(cmd_queue, mem_img_idxes, CL_TRUE, 0, SIZE_GLOBAL_IDX_TABLE, img_idxes, 0, NULL, NULL);
	verify(err == CL_SUCCESS);
