...
void show_codes_cuda(PVParallelView::PVBCICode<NBITS_INDEX>* device_codes, uint32_t n,
                     uint32_t width, uint32_t* device_img, uint32_t img_width, uint32_t x_start,
                     cudaStream_t stream = NULL);
void copy_img_cuda(uint32_t* host_img, uint32_t* device_img, uint32_t width);
void copy_codes_to_cuda(PVParallelView::PVBCICode<NBITS_INDEX>* host_codes,
                        PVParallelView::PVBCICode<NBITS_INDEX>* device_codes, size_t n,
                        cudaStream_t stream = NULL);
