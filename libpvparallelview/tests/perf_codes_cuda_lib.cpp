...
int main(int argc, char** argv)
{
	if (argc < 2) {
		...
		const char* const* patterns = PVParallelView::PVBCIPatterns<default_bbits>::get_patterns_string();
		std::cerr << "where pattern is one of the following:" << std::endl;
		for (int i = 0; i < PVParallelView::PVBCIPatterns<default_bbits>::get_number_patterns(); i++) {
