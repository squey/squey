...
class Vector1
{
  public:
	...
	C& back() const { return _array[_index - 1]; }
 ...
 */
enum { VectorImplIndex = 0, VectorImplSize };
...
class Vector2Impl
{
  public:
	...
	C* buffer() const { return (C*)(this + 1); }
...
class Vector2
{
  public:
	...
	unsigned size() const { return _v->index; }
	...
	inline bool is_null() { return (_v == 0); }

	inline C& at(const int i) { return _v->buffer()[i]; }

	inline C const& at(const int i) const { return _v->buffer()[i]; }
	...
	C& back() const { return _v->buffer()[_v->index - 1]; }
