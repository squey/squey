                     ...
                     uint32_t* buffer)
{
	...
	for (PVRow i = 0; i < packed_row_count; i += 4) {
		...
		const __m128i off_sse = _mm_and_si128(_mm_srli_epi32(y_sse, idx_shift), idx_mask_sse);
                         ...
                         int block_count)
{
	...
	for (PVRow i = 0; i < packed_row_count; i += 4) {
		...
		const __m128i res_sse =
		    _mm_and_si128(_mm_andnot_si128(_mm_cmplt_epi32(p_sse, _mm_set1_epi32(0)),
		                                   _mm_cmplt_epi32(p_sse, _mm_set1_epi32(block_count))),
		                  mask[f]);
		if (_mm_test_all_zeros(res_sse, _mm_set1_epi32(-1))) {
			continue;
                 ...
                 const int block_count)
{
	...
	BENCH_START(seq);
	uint32_t* res_seq = hist_plotted_seq(y_min, zoom, hist_seq, col_a, row_count, block_count);
	...
	BENCH_START(sse);
	uint32_t* res_sse = hist_plotted_sse(y_min, zoom, hist_sse, col_a, row_count, block_count);
	...
	BENCH_START(omp_sse);
	uint32_t* res_omp_sse =
	    hist_plotted_omp_sse(y_min, zoom, hist_omp_sse, col_a, row_count, block_count, omp_sse_ctx);
	...
	lib_omp.process_buffer_all(PVParallelView::PVHitGraphData::ProcessParams(
	    col_a, row_count, y_min, zoom, 10 /*value unused there*/, 1, 0, block_count));
              ...
              const int block_count)
{
	...
	BENCH_START(seq_v4);
	count_y1_sel_seq_v4(row_count, col_a, selection, y_min, zoom, hist_sel_seq_v4, block_count);
	BENCH_END(seq_v4, "hist_sel_seq_v4", row_count, sizeof(uint32_t), BUFFER_SIZE,
	          sizeof(uint32_t));

	uint32_t* hist_sel_sse_v4 = new uint32_t[real_buffer_size];
	memset(hist_sel_sse_v4, 0, real_buffer_size * sizeof(uint32_t));
	BENCH_START(sse_v4);
	count_y1_sel_sse_v4(row_count, col_a, selection, y_min, zoom, hist_sel_sse_v4, block_count);
	BENCH_END(sse_v4, "hist_sel_sse_v4", row_count, sizeof(uint32_t), BUFFER_SIZE,
	          sizeof(uint32_t));
	std::cout << "compare to ref: ";
	...
	BENCH_START(omp_v4);
	count_y1_sel_omp_v4(row_count, col_a, selection, y_min, zoom, hist_sel_omp_v4, block_count,
	                    omp_ctx);
	BENCH_END(omp_v4, "hist_sel_omp_v4", row_count, sizeof(uint32_t), BUFFER_SIZE,
	          sizeof(uint32_t));
	std::cout << "compare to ref: ";
	...
	count_y1_sel_omp_sse_v4(row_count, col_a, selection, y_min, zoom, hist_sel_omp_sse_v4,
	                        block_count, omp_ctx);
	BENCH_END(omp_sse_v4, "hist_sel_omp_sse_v4", row_count, sizeof(uint32_t), BUFFER_SIZE,
	          sizeof(uint32_t));
	std::cout << "compare to ref: ";
	...
	BENCH_START(seq_v5);
	count_y1_sel_seq_v5(row_count, col_a, selection, y_min, zoom, hist_sel_seq_v5, block_count);
	BENCH_END(seq_v5, "hist_sel_seq_v5", row_count, sizeof(uint32_t), BUFFER_SIZE,
	          sizeof(uint32_t));
	std::cout << "compare to ref: ";
	...
	PVParallelView::PVHitGraphData lib_omp(NBITS, block_count);
	BENCH_START(lib);
	lib_omp.process_buffer_selected(
	    PVParallelView::PVHitGraphData::ProcessParams(col_a, row_count, y_min, zoom,
	                                                  10 /*value unused there*/, 1, 0, block_count),
	    selection);
	BENCH_END(lib, "library-code", row_count, sizeof(uint32_t), BUFFER_SIZE, sizeof(uint32_t));
	std::cout << "compare to ref: ";
