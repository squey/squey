...
template <class DataContainer> class PVQuadTreeTmplBase
{
  public:
	...
	PVQuadTreeTmplBase(int max_level) : _max_level(max_level) { _datas.reserve(MAX_SIZE + 1); }
	...
	size_t memory_list() const { return _datas.memory(); }
	...
	void insert_list(const entry& e) { _datas.push_back(e); }
...
class PVQuadTreeTmpl : public PVQuadTreeTmplBase<DataContainer>
{
  public:
	...
	bool compare(PVQuadTree<DataContainer, Data>& qt)
	{
		if (this->_datas.is_null() && qt._datas.is_null()) {
			...
			// noeuds fils
			return _nodes[0].compare(*qt._nodes[0]) && _nodes[1].compare(*qt._nodes[1]) &&
			       _nodes[2].compare(*qt._nodes[2]) && _nodes[3].compare(*qt._nodes[3]);
  ...
  private:
	...
	void create_next_level()
	{
		...
		_nodes[NE].set(_y1_mid_value, _y1_max_value, _y2_mid_value, _y2_max_value,
		               this->_max_level - 1);

		_nodes[SE].set(_y1_mid_value, _y1_max_value, _y2_min_value, _y2_mid_value,
		               this->_max_level - 1);

		_nodes[SW].set(_y1_min_value, _y1_mid_value, _y2_min_value, _y2_mid_value,
		               this->_max_level - 1);

		_nodes[NW].set(_y1_min_value, _y1_mid_value, _y2_mid_value, _y2_max_value,
		               this->_max_level - 1);
...
class PVQuadTreeTmpl<DataContainer, Data, 0> : public PVQuadTreeTmplBase<DataContainer>
{
  public:
	...
	// PVQuadTreeTmpl(uint32_t y1_min_value, uint32_t y1_max_value, uint32_t
	// y2_min_value, uint32_t y2_max_value, int max_level) :
