...
class PVQuadTree
{
	...
	// typedef PVCore::PVVector<PVQuadTreeEntry,
	// tbb::scalable_allocator<PVQuadTreeEntry> > pvquadtree_entries_t;
  ...
  public:
	...
	class visit_tls
	{
	  public:
		...
		pv_tlr_buffer_t& get_tlr_buffer() { return _tlr_buffer; }

		pv_quadtree_buffer_entry_t* get_quadtree_buffer() { return &_quadtree_buffer[0]; }
	...
	class visit_context_t
	{
	  public:
		...
		visit_context_t(const uint64_t y_min, const uint64_t y_max, const insert_entry_f& insert_f)
		    : _y_min(y_min), _y_max(y_max), _insert_f(insert_f)
		{
		}

		uint64_t get_y_min() const { return _y_min; }

		uint64_t get_y_max() const { return _y_max; }

		const insert_entry_f& get_insert_f() const { return _insert_f; }

		tls_set_t& get_tls() const { return _tls; }
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (zoom == 0) {
				...
				if (obj._y1_mid_value < y1_max) {
					get_n_m(obj._nodes[NE], y1_min, y1_max, zoom - 1, y2_count >> 1, test_f,
					        insert_f, buffer, tlr);
					get_n_m(obj._nodes[SE], y1_min, y1_max, zoom - 1, y2_count >> 1, test_f,
					        insert_f, buffer, tlr);
				}
				if (y1_min < obj._y1_mid_value) {
					get_n_m(obj._nodes[NW], y1_min, y1_max, zoom - 1, y2_count >> 1, test_f,
					        insert_f, buffer, tlr);
					get_n_m(obj._nodes[SW], y1_min, y1_max, zoom - 1, y2_count >> 1, test_f,
					        insert_f, buffer, tlr);
				}
 ...
 */
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (y2_count == 1) {
				...
				if (obj._y1_mid_value < y1_max) {
					get_1_m(obj._nodes[NE], y1_min, y1_max, y2_count >> 1, test_f, insert_f, buffer,
					        tlr);
					get_1_m(obj._nodes[SE], y1_min, y1_max, y2_count >> 1, test_f, insert_f, buffer,
					        tlr);
				}
				if (y1_min < obj._y1_mid_value) {
					get_1_m(obj._nodes[NW], y1_min, y1_max, y2_count >> 1, test_f, insert_f, buffer,
					        tlr);
					get_1_m(obj._nodes[SW], y1_min, y1_max, y2_count >> 1, test_f, insert_f, buffer,
					        tlr);
				}
			} else if (obj._datas.size() != 0) {
 ...
 */
#ifdef QUADTREE_USE_SSE_EXTRACT
				extract_sse(obj, y1_min, y1_max, 0, y2_count, test_f, insert_f, buffer, tlr);
#else
				extract_seq(obj, y1_min, y1_max, 0, y2_count, test_f, insert_f, buffer, tlr);
#endif
			}
		}
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (zoom == 0) {
				...
				if (obj._y1_mid_value < y1_max) {
					get_n_1(obj._nodes[NE], y1_min, y1_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
					get_n_1(obj._nodes[SE], y1_min, y1_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
				}
				if (y1_min < obj._y1_mid_value) {
					get_n_1(obj._nodes[NW], y1_min, y1_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
					get_n_1(obj._nodes[SW], y1_min, y1_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
				}
			} else if (obj._datas.size() != 0) {
 ...
 */
#ifdef QUADTREE_USE_SSE_EXTRACT
				extract_sse(obj, y1_min, y1_max, zoom, 1, test_f, insert_f, buffer, tlr);
#else
				extract_seq(obj, y1_min, y1_max, zoom, 1, test_f, insert_f, buffer, tlr);
#endif
			}
		}
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (zoom == 0) {
				...
				if (obj._y2_mid_value < y2_max) {
					get_n_m(obj._nodes[NE], y2_min, y2_max, zoom - 1, y1_count >> 1, test_f,
					        insert_f, buffer, tlr);
					get_n_m(obj._nodes[NW], y2_min, y2_max, zoom - 1, y1_count >> 1, test_f,
					        insert_f, buffer, tlr);
				}
				if (y2_min < obj._y2_mid_value) {
					get_n_m(obj._nodes[SE], y2_min, y2_max, zoom - 1, y1_count >> 1, test_f,
					        insert_f, buffer, tlr);
					get_n_m(obj._nodes[SW], y2_min, y2_max, zoom - 1, y1_count >> 1, test_f,
					        insert_f, buffer, tlr);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of nxm
 ...
 */
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (y1_count == 1) {
				...
				if (obj._y2_mid_value < y2_max) {
					get_1_m(obj._nodes[NE], y2_min, y2_max, y1_count >> 1, test_f, insert_f, buffer,
					        tlr);
					get_1_m(obj._nodes[NW], y2_min, y2_max, y1_count >> 1, test_f, insert_f, buffer,
					        tlr);
				}
				if (y2_min < obj._y2_mid_value) {
					get_1_m(obj._nodes[SE], y2_min, y2_max, y1_count >> 1, test_f, insert_f, buffer,
					        tlr);
					get_1_m(obj._nodes[SW], y2_min, y2_max, y1_count >> 1, test_f, insert_f, buffer,
					        tlr);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of 1xm
 ...
 */
#ifdef QUADTREE_USE_SSE_EXTRACT
				extract_sse(obj, y2_min, y2_max, 0, y1_count, test_f, insert_f, buffer, tlr);
#else
				extract_seq(obj, y2_min, y2_max, 0, y1_count, test_f, insert_f, buffer, tlr);
#endif
			}
		}
		                    ...
		                    pv_tlr_buffer_t& tlr)
		{
			if (zoom == 0) {
				...
				if (obj._y2_mid_value < y2_max) {
					get_n_1(obj._nodes[NE], y2_min, y2_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
					get_n_1(obj._nodes[NW], y2_min, y2_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
				}
				if (y2_min < obj._y2_mid_value) {
					get_n_1(obj._nodes[SE], y2_min, y2_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
					get_n_1(obj._nodes[SW], y2_min, y2_max, zoom - 1, test_f, insert_f, buffer,
					        tlr);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of nx1
 ...
 */
#ifdef QUADTREE_USE_SSE_EXTRACT
				extract_sse(obj, y2_min, y2_max, zoom, 1, test_f, insert_f, buffer, tlr);
#else
				extract_seq(obj, y2_min, y2_max, zoom, 1, test_f, insert_f, buffer, tlr);
#endif
			}
		}
		...
		virtual tbb::task* execute()
		{
			if (_depth == task_depth_limit) {
				...
				if (_zoom == 0) {
					...
					job_t& job = *new (c->allocate_child()) job_t(_qt, _ctx, _y2_count, _depth + 1);
					c->set_ref_count(1);
					c->spawn(job);
				} else if (_y2_count == 1) {
					tbb::empty_task* c = new (allocate_continuation()) tbb::empty_task;
					...
					job_t& job = *new (c->allocate_child()) job_t(_qt, _ctx, _zoom, _depth + 1);
					c->set_ref_count(1);
					c->spawn(job);
					...
					if (_qt._y1_mid_value < _ctx.get_y_max()) {
						...
						jobs[0] = new (c->allocate_child()) job_visit_y1_n_m(
						    _qt._nodes[NE], _ctx, _zoom - 1, _y2_count >> 1, _depth + 1);

						jobs[1] = new (c->allocate_child()) job_visit_y1_n_m(
						    _qt._nodes[SE], _ctx, _zoom - 1, _y2_count >> 1, _depth + 1);
						num = 2;
					}
					if (_ctx.get_y_min() < _qt._y1_mid_value) {
						...
						jobs[num] = new (c->allocate_child()) job_visit_y1_n_m(
						    _qt._nodes[NW], _ctx, _zoom - 1, _y2_count >> 1, _depth + 1);

						++num;
						jobs[num] = new (c->allocate_child()) job_visit_y1_n_m(
						    _qt._nodes[SW], _ctx, _zoom - 1, _y2_count >> 1, _depth + 1);
						++num;
					}
				...
				if (_y2_count == 1) {
					...
					if (_qt._y1_mid_value < _ctx.get_y_max()) {
						...
						jobs[0] = new (c->allocate_child())
						    job_visit_y1_1_m(_qt._nodes[NE], _ctx, _y2_count >> 1, _depth + 1);
						jobs[1] = new (c->allocate_child())
						    job_visit_y1_1_m(_qt._nodes[SE], _ctx, _y2_count >> 1, _depth + 1);
						num = 2;
					}
					if (_ctx.get_y_min() < _qt._y1_mid_value) {
						if (!c)
						...
						jobs[num] = new (c->allocate_child())
						    job_visit_y1_1_m(_qt._nodes[NW], _ctx, _y2_count >> 1, _depth + 1);
						++num;
						jobs[num] = new (c->allocate_child())
						    job_visit_y1_1_m(_qt._nodes[SW], _ctx, _y2_count >> 1, _depth + 1);
						++num;
					}
			...
			if (_depth == task_depth_limit) {
				...
				if (_zoom == 0) {
					...
					if (_qt._y1_mid_value < _ctx.get_y_max()) {
						...
						jobs[0] = new (c->allocate_child())
						    job_visit_y1_n_1(_qt._nodes[NE], _ctx, _zoom - 1, _depth + 1);
						jobs[1] = new (c->allocate_child())
						    job_visit_y1_n_1(_qt._nodes[SE], _ctx, _zoom - 1, _depth + 1);
						num = 2;
					}
					if (_ctx.get_y_min() < _qt._y1_mid_value) {
						if (!c)
						...
						jobs[num] = new (c->allocate_child())
						    job_visit_y1_n_1(_qt._nodes[NW], _ctx, _zoom - 1, _depth + 1);
						++num;
						jobs[num] = new (c->allocate_child())
						    job_visit_y1_n_1(_qt._nodes[SW], _ctx, _zoom - 1, _depth + 1);
						++num;
					}
		                    ...
		                    const Ftest& test_f)
		{
			if (zoom == 0) {
				...
				if (obj._y1_mid_value < ctx.get_y_max()) {
					get_n_m(obj._nodes[NE], ctx, zoom - 1, y2_count >> 1, test_f);
					get_n_m(obj._nodes[SE], ctx, zoom - 1, y2_count >> 1, test_f);
				}
				if (ctx.get_y_min() < obj._y1_mid_value) {
					get_n_m(obj._nodes[NW], ctx, zoom - 1, y2_count >> 1, test_f);
					get_n_m(obj._nodes[SW], ctx, zoom - 1, y2_count >> 1, test_f);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of nxm
 * entries is needed
 */
		                    ...
		                    const Ftest& test_f)
		{
			if (y2_count == 1) {
				...
				if (obj._y1_mid_value < ctx.get_y_max()) {
					get_1_m(obj._nodes[NE], ctx, y2_count >> 1, test_f);
					get_1_m(obj._nodes[SE], ctx, y2_count >> 1, test_f);
				}
				if (ctx.get_y_min() < obj._y1_mid_value) {
					get_1_m(obj._nodes[NW], ctx, y2_count >> 1, test_f);
					get_1_m(obj._nodes[SW], ctx, y2_count >> 1, test_f);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of 1xm
 * entries is needed
 */
		                    ...
		                    const Ftest& test_f)
		{
			if (zoom == 0) {
				...
				if (obj._y1_mid_value < ctx.get_y_max()) {
					get_n_1(obj._nodes[NE], ctx, zoom - 1, test_f);
					get_n_1(obj._nodes[SE], ctx, zoom - 1, test_f);
				}
				if (ctx.get_y_min() < obj._y1_mid_value) {
					get_n_1(obj._nodes[NW], ctx, zoom - 1, test_f);
					get_n_1(obj._nodes[SW], ctx, zoom - 1, test_f);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of nx1
 * entries is needed
		                    ...
		                    const Ftest& test_f)
		{
			if (zoom == 0) {
				...
				if (obj._y2_mid_value < ctx.get_y_max()) {
					get_n_m(obj._nodes[NE], ctx, zoom - 1, y1_count >> 1, test_f);
					get_n_m(obj._nodes[NW], ctx, zoom - 1, y1_count >> 1, test_f);
				}
				if (ctx.get_y_min() < obj._y2_mid_value) {
					get_n_m(obj._nodes[SE], ctx, zoom - 1, y1_count >> 1, test_f);
					get_n_m(obj._nodes[SW], ctx, zoom - 1, y1_count >> 1, test_f);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of nxm
 * entries is needed
 */
		                    ...
		                    const Ftest& test_f)
		{
			if (y1_count == 1) {
				...
				if (obj._y2_mid_value < ctx.get_y_max()) {
					get_1_m(obj._nodes[NE], ctx, y1_count >> 1, test_f);
					get_1_m(obj._nodes[NW], ctx, y1_count >> 1, test_f);
				}
				if (ctx.get_y_min() < obj._y2_mid_value) {
					get_1_m(obj._nodes[SE], ctx, y1_count >> 1, test_f);
					get_1_m(obj._nodes[SW], ctx, y1_count >> 1, test_f);
				}
			} else if (obj._datas.size() != 0) {
/* this is a unsplitted node with data and an array of 1xm
 * entries is needed
 ...
 */
	    ...
	    PVQuadTree& new_tree, uint32_t y1_min, uint32_t y1_max, uint32_t y2_min, uint32_t y2_max)
	{
		if (_nodes != 0) {
			...
			if (_y1_mid_value < y1_max) {
				if (_y2_mid_value < y2_max) {
					_nodes[NE].get_subtree_from_y1y2(new_tree._nodes[NE], y1_min, y1_max, y2_min,
					                                 y2_max);
				}
				if (y2_min < _y2_mid_value) {
					_nodes[SE].get_subtree_from_y1y2(new_tree._nodes[SE], y1_min, y1_max, y2_min,
					                                 y2_max);
				}
			}
			if (y1_min < _y1_mid_value) {
				if (_y2_mid_value < y2_max) {
					_nodes[NW].get_subtree_from_y1y2(new_tree._nodes[NW], y1_min, y1_max, y2_min,
					                                 y2_max);
				}
				if (y2_min < _y2_mid_value) {
					_nodes[SW].get_subtree_from_y1y2(new_tree._nodes[SW], y1_min, y1_max, y2_min,
					                                 y2_max);
				}
			}
		} else {
