...
void show_codes_cuda(PVParallelView::PVBCICode<>* codes, uint32_t n, uint32_t width, GLuint buffer_id)
{
	...
	unsigned *pixels;
	size_t num_bytes;
	inendi_verify_cuda(cudaGraphicsGLRegisterBuffer(&cuda_pbo_resource, buffer_id, cudaGraphicsMapFlagsWriteDiscard));
	...
	inendi_verify_cuda(cudaGraphicsMapResources(1, &cuda_pbo_resource, 0));
	inendi_verify_cuda(cudaGraphicsResourceGetMappedPointer((void**)&pixels, &num_bytes, cuda_pbo_resource));
