...
int main()
{
	...
	BENCH_END(random_points, "random_points", sizeof(random_point_t), PIXEL_COUNT, sizeof(uint32_t),
	          PIXEL_COUNT);
	QString image_path = "/tmp/random_points.bmp";
	bool res = img.save(image_path, "bmp");
	std::cout << "image saved to '" << qPrintable(image_path) << "': " << std::boolalpha << res
	          << std::endl;
	...
	BENCH_END(random_points_8bits, "random_points_8bits", sizeof(random_point_t), PIXEL_COUNT,
	          sizeof(char), PIXEL_COUNT);
}
