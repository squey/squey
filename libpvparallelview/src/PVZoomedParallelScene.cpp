    ...
    , _view_deleted(false)
{
	...
	_sliders_group = new PVParallelView::PVSlidersGroup(_sliders_manager_p, _axis_id.get_axis());
...
PVParallelView::PVZoomedParallelScene::~PVZoomedParallelScene()
{
	...
	if (_pending_deletion == false) {
		...
		_sliders_manager_p->del_zoom_sliders(_axis_id.get_axis(), _sliders_group);
	}
...
void PVParallelView::PVZoomedParallelScene::update_display()
{
	...
	_sliders_manager_p->update_zoom_sliders(_axis_id.get_axis(), _sliders_group, y_min, y_max,
    ...
    PVSlidersManager::ZoomSliderChange change)
{
	...
	if ((nraw_col == _axis_id.get_axis()) && (id == _sliders_group)) {
		if (y_max < y_min) {
