                     ...
                     const HCSSE_ALPHA_DOUBLE_VEC alpha_sse, const __m128i zoom_mask_sse, uint32_t idx_shift, uint32_t zoom_shift, size_t nbits)
{
	...
	const __m128d tmp2_lo_sse = inendi_mm_cvtepu32_pd(tmp1_lo_sse);
	const __m128d tmp2_hi_sse = inendi_mm_cvtepu32_pd(tmp1_hi_sse);
	const __m128d tmp3_lo_sse = _mm_mul_pd(tmp2_lo_sse, alpha_sse);
	const __m128d tmp3_hi_sse = _mm_mul_pd(tmp2_hi_sse, alpha_sse);
	const __m128i tmp4_lo_sse = inendi_mm_cvttpd_epu32(tmp3_lo_sse);
	const __m128i tmp4_hi_sse = inendi_mm_cvttpd_epu32(tmp3_hi_sse);
