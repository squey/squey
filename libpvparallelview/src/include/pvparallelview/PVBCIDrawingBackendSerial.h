...
class PVBCIDrawingBackendSerial : protected Engine
{
	...
	typedef PVCore::PVTypeTraits::function_traits<decltype(&PVBCIDrawingBackend<11>::operator())>
	    backend11_functor_traits_t;
	...
	struct drawing_job {
		...
		drawing_job(drawing_job const& o) : _bbits(o._bbits)
		{
  ...
  public:
	PVBCIDrawingBackendSerial() : Engine() { _cur_grp = -1; }
