...
while true; do
    # Run INENDI Inspector with "--allow=devel" option to be able to generate crash reports
    ...
    output=`flatpak run --allow=devel --env="PYTHONPATH=/opt/inendi/inspector/python${PYTHON_VERSION}/site-packages" com.gitlab.inendi.Inspector//$b 2>&1`
    ret=$?
