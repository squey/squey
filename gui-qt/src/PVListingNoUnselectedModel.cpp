...
//! $Id: PVListingNoUnselectedModel.cpp 3248 2011-07-05 10:15:19Z rpernaudat $
...
QVariant PVInspector::PVListingNoUnselectedModel::data(const QModelIndex &index, int role) const
{
	...
	real_row_index = lib_view->get_nu_real_row_index(correspondTable.at(index.row()));
...
void PVInspector::PVListingNoUnselectedModel::initCorrespondance(){
	...
	for(int i=0;i<rowCount(QModelIndex());i++){
		correspondTable.insert(i,lib_view->get_nu_real_row_index(i));
	}
	sortOrder = NoOrder;
}
