 ...
 *
 *****************************************************************************/
PVInspector::PVExtractorWidget::PVExtractorWidget(Inendi::PVSource& lib_src,
                                                  PVGuiQt::PVProjectsTabWidget* projects_tab,
                                                  QWidget* parent)
    : QDialog(parent), _lib_src(&lib_src), _projects_tab(projects_tab)
{
	...
	_list_inputs->setHeaderLabels(QStringList() << "Source"
	...
	// connect(_slider_index, SIGNAL(sliderReleased()), this,
	// SLOT(slider_released_Slot()));
	connect(_size_batch_widget, SIGNAL(textEdited(QString const&)), this,
	        SLOT(size_batch_edited_Slot(QString const&)));
	connect(_source_starts_line, SIGNAL(textEdited(QString const&)), this,
	        SLOT(line_start_edited_Slot(QString const&)));
...
void PVInspector::PVExtractorWidget::fill_source_list()
{
	...
	for (it = in.begin(); it != in.end(); it++) {
		...
		items.append(new QTreeWidgetItem(
		    (QTreeWidget*)NULL, QStringList() << (*it)->human_name() << QString("%1").arg(nlines)));
	}
...
void PVInspector::PVExtractorWidget::update_status_ext(PVCore::PVProgressBox* pbox,
                                                           ...
                                                           QWidget* parent = NULL)
{
	PVCore::PVProgressBox* pbox = new PVCore::PVProgressBox(
	    tr("Extracting %1...").arg(desc), parent, 0, QString("Number of elements extracted: %L1"));
...
void PVInspector::PVExtractorWidget::process_Slot()
{
	...
	bool success = PVExtractorWidget::show_job_progress_bar(job, ext.get_format().get_format_name(),
	                                                        job->nb_elts_max(), this);
