...
# to only get versionning dependencies, use :
#    find . -name "*.so" | LANG=C xargs readelf -V |awk '$2 == "Name:" {print $3}' |sort -V |uniq -c

# for simple dot generation
#     find  /usr/lib64/scilab/ -name "*.so.5.5.1" | LANG=C xargs readelf -d |awk 'BEGIN{FS="\\(|\\)|\\[|\\]"; delete stash[0]; print "digraph DependencyTree {"; print "  ranksep=4;"} END{print "}"} $2=="NEEDED" {stash[length(stash)]=$4;}; $2=="SONAME" {for (i in stash) {print "  \""stash[i]"\" -> \""$4"\""; delete stash[i]; }}' |dot -Tpng -osample.png
