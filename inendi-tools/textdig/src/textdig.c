...
int main(int argc, char **argv)
{
	...
	if (!options.interactive_console) {
		...
		for (i=0; i < options.n_files; i++) {
			inendi_debug(INENDI_DEBUG_NOTICE, "Processing %s...\n", options.files[i]);

			file = inendi_file_new(options.files[i]);
			options.current_line_count = 0;
			options.current_index = 0;
			...
			if (options.n_line_print) {
		        	ret = inendi_file_line_foreach(file, print_line_x_function, &options);
			}

			if (options.n_split) {
		        	ret = inendi_file_line_foreach(file, split_lines_function, &options);
	        		fclose(options.fd);
			}
			...
			inendi_debug(INENDI_DEBUG_NOTICE, "Done!\n");
		...
		if (!ret) {
			inendi_debug(INENDI_DEBUG_CRITICAL, "Cannot start the interractive console!\n");
			inendi_terminate();
			return 1;
		}
	}
