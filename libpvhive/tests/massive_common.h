...
class Block
{
  public:
	...
	void set_prop(int i, Property p) { _array[i] = p; }

	const Property& get_prop(int i) const { return _array[i]; }

	Property& get_prop(int i) { return _array[i]; }
...
class BlockObs : public PVHive::PVObserver<Block>
{
  public:
	...
	void refresh() { _value = get_object()->get_value(); }

	void about_to_be_deleted() { _quit = true; }
