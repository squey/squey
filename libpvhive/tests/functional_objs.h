...
class Property : public Storage
{
  public:
	...
	void set_parent(Entity* p) { _parent = p; }

	Entity* get_parent() const { return _parent; }
...
class Entity : public Storage
{
  public:
	...
	QString get_id_str() const { return QString::number(_id); }
	...
	virtual Property const* get_prop() const { return nullptr; }

	virtual Property* get_prop() { return nullptr; }
...
{
  public:
	PropertyEntity(int id, Entity* parent = nullptr) : Entity(id, parent), _prop()
	{
	...
	virtual bool has_prop() const { return true; }

	void set_prop(const Property& p) { _prop = p; }

	virtual Property const* get_prop() const { return &_prop; }
...
class ThreadEntity : public QThread
{
  ...
  public:
	...
	Storage_p get_ent() const { return _s; }
...
{
  public:
	EntityActor(int id) : Interactor(id), _value(0) {}
...
{
  public:
	PropertyEntityActor(int id) : Interactor(id), _value(0) {}
...
class EntityButtonActor : public QDialog, public EntityActor
{
  ...
  public:
	EntityButtonActor(int id, Storage* s, QWidget* parent) : QDialog(parent), EntityActor(id)
	{
...
class PropertyButtonActor : public QDialog, public PropertyEntityActor
{
  ...
  public:
	PropertyButtonActor(int id, Storage* s, QWidget* parent)
	    : QDialog(parent), PropertyEntityActor(id)
	{
		QVBoxLayout* vb = new QVBoxLayout(this);
...
{
  public:
	EntityThreadActor(int id, QObject* parent) : QThread(parent), EntityActor(id), _can_run(true)
	{
	...
	void run()
	{
		while (_can_run) {
			msleep(_duration);
			std::cout << "thread actor " << get_id() << ": update to " << _value << std::endl;
			PVACTOR_CALL(*this, &Entity::set_value, _value);
			++_value;
		}
	}
...
class PropertyThreadActor : public QThread, public PropertyEntityActor
{
  public:
	PropertyThreadActor(int id, QObject* parent)
	    : QThread(parent), PropertyEntityActor(id), _can_run(true)
	{
		_duration = random() % 2000;
		start();
	}
	...
	void run()
	{
		while (_can_run) {
			msleep(_duration);
			std::cout << "thread actor " << get_id() << ": update to " << _value << std::endl;
			call<decltype(&PropertyEntity::set_prop), &PropertyEntity::set_prop>(Property(_value));
			++_value;
		}
	}
...
class EntityObserver : public QDialog, public Interactor, public PVHive::PVObserver<Entity>
{
  ...
  public:
	EntityObserver(int id, Storage_p& s, QWidget* parent) : QDialog(parent), Interactor(id)
	{
		QVBoxLayout* vb = new QVBoxLayout(this);
		...
		setWindowTitle("entity observer " + QString().sprintf("%p", this) + " - " + s->get_name());
	...
	~EntityObserver() { std::cout << "entity observer " << get_id() << ": death" << std::endl; }

	void refresh() { _vl->setText("value: " + QString::number(get_object()->get_value())); }
...
class PropertyObserver : public QDialog, public Interactor, public PVHive::PVObserver<Property>
{
  ...
  public:
	PropertyObserver(int id, Storage_p& e, QWidget* parent) : QDialog(parent), Interactor(id)
	{
		QVBoxLayout* vb = new QVBoxLayout(this);
	...
	~PropertyObserver() { std::cout << "property observer " << get_id() << ": death" << std::endl; }

	void refresh()
	{
		_vl->setText("value (0x" + QString::number((long)get_object(), 16) + "): " +
		             QString::number(get_object()->get_value()));
	}
...
class EntityQObserver : public PVHive::PVQObserver<Entity>, public Interactor
{
  ...
  public:
	EntityQObserver(int id, QObject* parent) : PVHive::PVQObserver<Entity>(parent), Interactor(id)
	{
	}
	...
	{
		std::cout << "entity qobserver " << get_id() << ": value: " << get_object()->get_value()
		          << std::endl;
	}
...
class PropertyQObserver : public PVHive::PVQObserver<Property>, public Interactor
{
  ...
  public:
	PropertyQObserver(int id, QObject* parent)
	    : PVHive::PVQObserver<Property>(parent), Interactor(id)
	{
	}
	...
	virtual void do_refresh(PVHive::PVObserverBase*)
	{
		std::cout << "property qobserver " << get_id() << ": value: " << get_object()->get_value()
		          << std::endl;
...
class EntityObserverCB : public QObject, public Interactor
{
  ...
  public:
	...
	EntityObserverCB(int id) : Interactor(id)
	{
...
class EntityObserverSignal : public QObject, public Interactor
{
  ...
  public:
	EntityObserverSignal(int id, QObject* parent) : QObject(parent), Interactor(id), _os(this)
	{
		_os.connect_refresh(this, SLOT(do_refresh(PVHive::PVObserverBase*)));
		_os.connect_about_to_be_deleted(this,
		                                SLOT(do_about_to_be_deleted(PVHive::PVObserverBase*)));
	}
	...
	PVHive::PVObserverBase* get() { return &_os; }
  ...
  public slots:
	void do_refresh(PVHive::PVObserverBase*)
	{
		std::cout << "entity observersignal " << get_id()
		          << ": value: " << _os.get_object()->get_value() << std::endl;
	}
...
class PropertyObserverSignal : public QObject, public Interactor
{
  ...
  public:
	PropertyObserverSignal(int id, QObject* parent) : QObject(parent), Interactor(id), _os(this)
	{
		_os.connect_refresh(this, SLOT(do_refresh(PVHive::PVObserverBase*)));
		_os.connect_about_to_be_deleted(this,
		                                SLOT(do_about_to_be_deleted(PVHive::PVObserverBase*)));
