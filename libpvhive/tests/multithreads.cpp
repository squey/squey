...
int main()
{
	...
	///////////////////////////////////////////////////
	{
		...
		boost::thread th1(boost::bind(thread1, boost::ref(obj), boost::ref(actor),
		                              boost::ref(nb_sucess), boost::ref(nb_exceptions)));
		...
		test2_multithread_no_object_exception_raised =
		    (nb_exceptions == nb_unregistered || nb_exceptions == nb_unregistered - 1) &&
		    (nb_sucess + nb_exceptions == MAX_COUNT);

		PVLOG_INFO("no_object multithreaded exception passed: %d (success:%d / "
		           "exceptions:%d / total:%d, unregister:%d)\n",
		           test2_multithread_no_object_exception_raised, nb_sucess, nb_exceptions,
		           MAX_COUNT, nb_unregistered);
	}

	return !(test1_monothread_no_object_exception_raised &&
	         test2_multithread_no_object_exception_raised);
}
