...
void PVParallelView::PVSlidersManager::del_zoom_sliders(const PVCol axis,
                                                        const id_t id)
{
	interval_geometry_set_t::iterator ai = _zoom_geometries.find(axis);
...
void PVParallelView::PVSlidersManager::update_zoom_sliders(const PVCol axis,
                                                           ...
                                                           const uint32_t y_max)
{
	interval_geometry_t &geom = _zoom_geometries[axis][id];
	geom.y_min = y_min;
	geom.y_max = y_max;
}
...
{
	for(interval_geometry_set_t::const_iterator it = _zoom_geometries.begin();
