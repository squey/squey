...
void PVParallelView::PVSlidersManager::new_zoom_sliders(const axe_id_t &axe_id,
                                                        const id_t id,
                                                        const uint32_t y_min,
                                                        const uint32_t y_max)
{
	new_range_sliders(_zoom_geometries, axe_id, id, y_min, y_max);
}
...
void PVParallelView::PVSlidersManager::new_selection_sliders(const axe_id_t &axe_id,
                                                             const id_t id,
                                                             const uint32_t y_min,
                                                             const uint32_t y_max)
{
	new_range_sliders(_selection_geometries, axe_id, id, y_min, y_max);
}
...
void PVParallelView::PVSlidersManager::del_zoom_sliders(const axe_id_t &axe_id,
                                                        const id_t id)
{
	del_range_sliders(_zoom_geometries, axe_id, id);
}
...
void PVParallelView::PVSlidersManager::del_selection_sliders(const axe_id_t &axe_id,
                                                             const id_t id)
{
	del_range_sliders(_selection_geometries, axe_id, id);
}
...
void PVParallelView::PVSlidersManager::update_zoom_sliders(const axe_id_t &axe_id,
                                                           const id_t id,
                                                           const uint32_t y_min,
                                                           const uint32_t y_max,
                                                           const ZoomSliderChange)
{
	...
	update_range_sliders(_zoom_geometries, axe_id, id, y_min, y_max);
}
...
void PVParallelView::PVSlidersManager::update_selection_sliders(const axe_id_t &axe_id,
                                                                const id_t id,
                                                                const uint32_t y_min,
                                                                const uint32_t y_max)
{
	update_range_sliders(_selection_geometries, axe_id, id, y_min, y_max);
}
...
void PVParallelView::PVSlidersManager::new_range_sliders(range_geometry_set_t &range,
                                                         ...
                                                         const uint32_t y_max)
{
	update_range_sliders(range, axe_id, id, y_min, y_max);
}
...
void PVParallelView::PVSlidersManager::del_range_sliders(range_geometry_set_t &range,
                                                         ...
                                                         const id_t id)
{
	range_geometry_set_t::iterator ai = range.find(axe_id);
	if (ai != range.end()) {
...
void PVParallelView::PVSlidersManager::update_range_sliders(range_geometry_set_t &range,
                                                            ...
                                                            const uint32_t y_max)
{
	range_geometry_t &geom = range[axe_id][id];
	geom.y_min = y_min;
	geom.y_max = y_max;
}
