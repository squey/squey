...
__global__ void bcicode_raster_unroll2(uint2* bci_codes, unsigned int n, unsigned int width, unsigned int* img_dst/*[width][IMAGE_HEIGHT]*/)
{
	...
	const int band_x_start = threadIdx.x + blockIdx.x*blockDim.x;
	if (band_x_start >= width) {
		return;
	}
	const unsigned int size_grid_x = blockDim.x*blockDim.y;
	...
	for (; idx_codes < n_end; idx_codes += size_grid2) {
		...
		for (int band_x = band_x_start; band_x < width; band_x += size_grid_x) {
