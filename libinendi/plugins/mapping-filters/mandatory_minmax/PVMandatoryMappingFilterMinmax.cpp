...
{
	template <typename T>
	static void call(Inendi::PVMapped const* mapped, Inendi::mandatory_param_list_values const& values, Inendi::mandatory_param_map* mandatory_params)
	{
		...
		Inendi::mandatory_param_value ymin(mapped->get_parent()->get_value(mmar.get_min_index(), values.first), ds);
		ds.storage_cast<T>() = mmar.get_max_value();
		Inendi::mandatory_param_value ymax(mapped->get_parent()->get_value(mmar.get_max_index(), values.first), ds);

		mandatory_params->insert(Inendi::mandatory_param_map::value_type(Inendi::mandatory_ymin, ymin));
		mandatory_params->insert(Inendi::mandatory_param_map::value_type(Inendi::mandatory_ymax, ymax));
	}
};
