...
static constexpr const char* csv_file_format = TEST_FOLDER "/picviz/ipv4_default_mapping.csv.format";
...
int main()
{
    ...
    PV_ASSERT_VALID(std::adjacent_find(distinct_mapping.begin(), distinct_mapping.end(),
		    [](uint32_t a, uint32_t b) { return std::abs(a - b) > 1; }) != distinct_mapping.end());
#endif
