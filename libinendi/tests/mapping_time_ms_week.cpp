...
static constexpr const char* csv_file_format = TEST_FOLDER "/picviz/datetime_ms_week_mapping.csv.format";
...
int main()
{
    ...
    std::sort(order.begin(), order.end(), [&](uint32_t a, uint32_t b) {
	    ...
	    return  cal->get(UCAL_DAY_OF_WEEK, err) < cal_2->get(UCAL_DAY_OF_WEEK, err) or
		   (cal->get(UCAL_DAY_OF_WEEK, err) == cal_2->get(UCAL_DAY_OF_WEEK, err) and cal->get(UCAL_HOUR_OF_DAY, err) < cal_2->get(UCAL_HOUR_OF_DAY, err)) or
		   (cal->get(UCAL_DAY_OF_WEEK, err) == cal_2->get(UCAL_DAY_OF_WEEK, err) and cal->get(UCAL_HOUR_OF_DAY, err) == cal_2->get(UCAL_HOUR_OF_DAY, err) and cal->get(UCAL_MINUTE, err) < cal_2->get(UCAL_MINUTE, err)) or
		   (cal->get(UCAL_DAY_OF_WEEK, err) == cal_2->get(UCAL_DAY_OF_WEEK, err) and cal->get(UCAL_HOUR_OF_DAY, err) == cal_2->get(UCAL_HOUR_OF_DAY, err) and cal->get(UCAL_MINUTE, err) == cal_2->get(UCAL_MINUTE, err) and cal->get(UCAL_SECOND, err) < cal_2->get(UCAL_SECOND, err));
	   });
