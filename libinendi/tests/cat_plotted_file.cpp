...
int main(int argc, char** argv)
{
	...
	for (size_t i = 0; i < n; i++) {
		...
		if (transposed == -1) {
			...
			if (transposed != header.is_transposed) {
				std::cerr << "All files must be transposed or not, not a mix of both !"
				          << std::endl;
				return 1;
			}
	...
	if (!fout) {
		std::cerr << "Unable to open " << argv[argc - 1] << " for writing: " << strerror(errno)
		          << std::endl;
		return 1;
	}
			...
			for (size_t i = 0; i < n; i++) {
				...
				for (; cur_r < (nrows[i] / SIZE_TMP) * SIZE_TMP; cur_r += SIZE_TMP) {
					if (fread(tmp_buf, sizeof(float), SIZE_TMP, cur_f) != SIZE_TMP) {
						std::cerr << "Error reading " << argv[i + 1] << ": " << strerror(errno)
						          << std::endl;
						return 1;
					}
