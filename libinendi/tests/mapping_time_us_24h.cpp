...
static constexpr const char* csv_file_format = TEST_FOLDER "/picviz/datetime_us_24h_mapping.csv.format";
...
int main()
{
    ...
    std::sort(order.begin(), order.end(), [&](uint32_t a, uint32_t b) {
	    const boost::posix_time::ptime ta = *reinterpret_cast<const boost::posix_time::ptime*>(&array[a]);
	    const boost::posix_time::ptime tb = *reinterpret_cast<const boost::posix_time::ptime*>(&array[b]);
	    tm tm_a = to_tm(ta);
	    tm tm_b = to_tm(tb);
	    return tm_a.tm_hour < tm_b.tm_hour or
		   (tm_a.tm_hour == tm_b.tm_hour and tm_a.tm_min < tm_b.tm_min) or
		   (tm_a.tm_hour == tm_b.tm_hour and tm_a.tm_min == tm_b.tm_min and tm_a.tm_sec < tm_b.tm_sec); });
