...
Inendi::PVSparseSelection& Inendi::PVSparseSelection::operator&=(PVSparseSelection const& o)
{
	...
	map_chunks_t::iterator it_this;
	map_chunks_t::const_iterator it_o;
	it_this = _chunks.begin();
	it_o = o._chunks.begin();
	...
	while (it_this != _chunks.end()) {
		...
		if (idx_o == idx_this) {
			// We found the current index of our object in the other one. Let's do the
			// merge.
			...
			map_chunks_t::iterator it_next = it_this;
			it_next++;
...
Inendi::PVSparseSelection& Inendi::PVSparseSelection::operator|=(PVSparseSelection const& o)
{
	...
	map_chunks_t::iterator it_this, it_before_this;
	map_chunks_t::const_iterator it_o;
	it_this = _chunks.begin();
	it_o = o._chunks.begin();
	...
	while (it_this != _chunks.end()) {
		...
		while (idx_o < idx_this) {
			...
			// The element position that precedes this new element in our object is
			// it_before_this.
