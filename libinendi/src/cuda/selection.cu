...
/******************************************************************************
 *
 * inendi_selection_count
 *
 *****************************************************************************/
/* We need to provide lines_counts because select all will select even lines that do not exists! */
pv_row inendi_selection_count(inendi_selection_t *selection, pv_row last_line_index)
{
	pv_row index;
	pv_row count;
	...
	for (index = 0; index <= last_line_index; index++) {
		count += inendi_selection_get_line(selection, index);
	}
...
void inendi_selection_AB2A_or(inendi_selection_t *a, inendi_selection_t *b)
{
	...
	seltable_s = INENDI_SELECTION_NUMBER_OF_CHUNKS * sizeof(uint32_t);
	...
	inendi_selection_AB2C_or_k<<<6,256>>>((uint32_t *)da, (uint32_t *)db, (uint32_t *)dc);
