...
#ifndef INENDI_PVSPARSESELECTION_H
#define INENDI_PVSPARSESELECTION_H
...
class PVSparseSelection
{
  ...
  private:
	...
	inline chunk_t& get_chunk_by_idx(const chunk_index_t idx)
	{
		...
		if (it_c == _chunks.end()) {
			// This chunk does not exist. Add a new one. Specify the last used chunk as a helper in
			// order
	...
	inline static chunk_index_t bit_to_chunk_index(size_t bit)
	{
		return bit >> nbits_per_chunk_ln2;
	}
