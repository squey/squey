...
#ifndef INENDI_PVVIEW_IMPL_H
#define INENDI_PVVIEW_IMPL_H
...
private:
	...
	PVCol column;
	Inendi::PVSortingFunc_f f;
};

template <class Tint>
struct PVViewStableSortDesc: public PVViewSortBuf
{
	PVViewStableSortDesc(PVRush::PVNraw const* nraw_, PVCol col, Inendi::PVSortingFunc_f f_): 
		PVViewSortBuf(), nraw(nraw_), column(col), f(f_)
	{ }
	bool operator()(Tint idx1, Tint idx2) const
	{
...
{
	PVViewCompEquals(PVRush::PVNraw const* nraw_, PVCol col, Inendi::PVSortingFunc_fequals f_equals): 
...
private:
	PVRush::PVNraw const* nraw;
	PVCol column;
	Inendi::PVSortingFunc_fequals f;
};
...
{
	PVMultisetSortAsc(Inendi::PVSortingFunc_f f) : _f(f) {}
...
private:
	Inendi::PVSortingFunc_f _f;
};

template <typename Tkey>
struct PVMultisetSortDesc
{
	PVMultisetSortDesc(Inendi::PVSortingFunc_f f) : _f(f) {}
...
template <class L>
void stable_sort_indexes_f(PVRush::PVNraw const* nraw, PVCol col, Inendi::PVSortingFunc_f f, Qt::SortOrder order, L& idxes, tbb::task_group_context* ctxt = NULL)
{
...
template <class L>
void unique_indexes_copy_f(PVRush::PVNraw const* nraw, PVCol col, Inendi::PVSortingFunc_fequals f_equals, L const& idxes_in, L& idxes_out)
{
...
template <class L>
typename L::iterator unique_indexes_f(PVRush::PVNraw const* nraw, PVCol col, Inendi::PVSortingFunc_fequals f_equals, L& idxes_in)
{
	PVViewCompEquals<typename L::value_type> e(nraw, col, f_equals);
