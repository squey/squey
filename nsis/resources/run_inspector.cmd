...
wsl -d inspector_linux --user inendi --exec bash -c "%inspector_path_linux%/setup_config_dir.sh %appdata_path_linux%; flatpak run --device=shm --nofilesystem=/tmp --env='WSL_USERPROFILE=%userprofile_path%' --env='QTWEBENGINE_CHROMIUM_FLAGS=--disable-dev-shm-usage' --command=bash %1 -c '%DISPLAY_CONFIG% /app/bin/inendi-inspector'"

echo "Stop VcXsrv if needed"
if [%WSLG_VERSION%]==[] (
