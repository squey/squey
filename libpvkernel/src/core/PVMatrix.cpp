...
{
	__m128 sse_row0, sse_row1, sse_row2, sse_row3;
	...
	for (uint32_t i = 0; i < (nrows/4)*4; i += 4) {
		...
		for (uint32_t j = 0; j < (ncols/4)*4; j += 4) {
			...
			_MM_TRANSPOSE4_PS(sse_row0, sse_row1, sse_row2, sse_row3);
		...
		for (uint32_t j = (ncols/4)*4; j < ncols; j++) {
			__m128 sse_final = _mm_set_ps(data[(i+3)*ncols+j], data[(i+2)*ncols+j], data[(i+1)*ncols+j], data[(i+0)*ncols+j]);
