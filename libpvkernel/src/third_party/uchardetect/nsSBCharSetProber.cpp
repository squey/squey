	...
	for (PRUint32 i = 0; i < aLen; i++) {
		...
		if (order < mModel->freqCharCount) {
			...
			if (mLastOrder < mModel->freqCharCount) {
				mTotalSeqs++;
				if (!mReversed)
					++(mSeqCounters[mModel->precedenceMatrix[mLastOrder * mModel->freqCharCount +
					                                         order]]);
				else // reverse the order of the letters in the lookup
					++(mSeqCounters[mModel->precedenceMatrix[order * mModel->freqCharCount +
					                                         mLastOrder]]);
			}
		}
		mLastOrder = order;
...
float nsSingleByteCharSetProber::GetConfidence(void)
{
	...
	if (mTotalSeqs > 0)
		if (mTotalSeqs > mSeqCounters[NEGATIVE_CAT] * 10)
			return ((float)(mTotalSeqs - mSeqCounters[NEGATIVE_CAT] * 10)) / mTotalSeqs *
			       mFreqChar / mTotalChar;
	return (float)0.01;
