...
inline static int inendi_mm_getpos_max_epi16(__m128i v, __m128i const sse_ff)
{
	// Get ~v
...
inline static int inendi_mm_getpos_nonzero_epi8(__m128i v, __m128i const sse_ff)
{
	// Get ~v
	v = _mm_andnot_si128(v, sse_ff);
...
inline static int inendi_mm_getpos_lastnonzero_epi8(__m128i const ssev)
{
...
#ifdef __AVX__
inline static __m256d inendi_mm256_cvtepu32_pd(__m128i const v)
{
...
inline static __m128i inendi_mm256_cvttpd_epu32(__m256d const v)
{
...
#ifdef __SSE2__
inline static __m128d inendi_mm_cvtepu32_pd(__m128i const v)
{
	const __m128i mask_carry_sse = _mm_set1_epi32(0x7fffffff);
...
inline static __m128i inendi_mm_cvttpd_epu32(__m128d const v)
{
 ...
 *"less-than"
 * The result using _mm_cmplt_epi32 (signed version) is wrong iif only one of the two operand is
 *negative.
 * This function fixes that issue.
 *
 * res[i] = (a[i] < b[i])
 */
inline __m128i inendi_mm_cmplt_epu32(__m128i const a, __m128i const b)
{
	...
	return reinterpret_cast<__m128i>(_mm_blendv_ps(
	    reinterpret_cast<__m128>(cmp_signed),                          // if mask[i][31] == 0
 ...
 */
inline __m128i inendi_mm_cmprange_epi32(__m128i const v, __m128i const a, __m128i const b)
{
	// _mm_andnot_si128(a,b) = ~a & b
	// _mm_cmplt_epi32(a,b) = a < b;
	// thus andnot(cmplt(a,b),cmplt(a,c)) <=> (!(a < b)) && (a < c) <=> (a >=b) && (a < c)
	return _mm_andnot_si128(_mm_cmplt_epi32(v, a), _mm_cmplt_epi32(v, b));
}
 ...
 * res[i] = (v[i] >= a[i]) && (v[i] < b[i])
 */
inline __m128i inendi_mm_cmprange_in_epi32(__m128i const v, __m128i const a, __m128i const b)
{
	// same logic as previous function
	return _mm_andnot_si128(_mm_cmplt_epi32(v, a),
	                        _mm_or_si128(_mm_cmplt_epi32(v, b), _mm_cmpeq_epi32(v, b)));
}
 ...
 * res[i] = (v[i] >= a[i]) && (v[i] < b[i])
 */
inline static __m128i inendi_mm_cmprange_epu32(__m128i const v, __m128i const a, __m128i const b)
{
	return _mm_andnot_si128(inendi_mm_cmplt_epu32(v, a), inendi_mm_cmplt_epu32(v, b));
}
 ...
 * res[i] = (v[i] >= a[i]) && (v[i] <= b[i])
 */
inline static __m128i inendi_mm_cmprange_in_epu32(__m128i const v, __m128i const a, __m128i const b)
{
	return _mm_andnot_si128(inendi_mm_cmplt_epu32(v, a),
	                        _mm_or_si128(inendi_mm_cmplt_epu32(v, b), _mm_cmpeq_epi32(v, b)));
}
 ...
 */
inline static int32_t inendi_mm_hmin_epi32(__m128i const v)
{
 ...
 * \return min({v[i], i=0..4})
 */
inline static uint32_t inendi_mm_hmin_epu32(__m128i const v)
{
	__m128i min_perm = _mm_shuffle_epi32(v, (2 | (3 << 2)));
