...
#define BENCH_SHOW(var, desc, nelts_start, size_elt_start, nelts_end, size_elt_end)                \
	{                                                                                              \
		...
		std::cout << desc << ": in " << time * 1000 << " ms. Input (#/size/BW): " << nelts_start   \
		          << "/" << size_in_mb << " MB/" << bw_in                                          \
		          << " MB/s | Output (#/size/BW): " << nelts_end << "/" << size_out_mb << " MB/"   \
		          << bw_out << " MB/s" << std::endl;                                               \
	}
...
#define MEM_END(var, desc)                                                                         \
	...
	std::cout << desc << ": memory footprint is: VM=" << diff_vm_mb << " MB / RES=" << diff_rss_mb \
	          << " MB" << std::endl;
