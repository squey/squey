 ...
 * </ul>
 *
 * In memory, it is organised this way (see also \ref PVChunkMem): each PVChunkMem object is
 *allocated in a way that it contains
 * space after its own structure for its data. So, a pointer to a PVChunkMem object actually points
 *to:
 * <pre>
 ...
 * </pre>
 *
 * To have a direct pointer to the associated data of a PVChunk, use the \ref begin method. PVChunk
 *is just an interface implemented by PVChunkMem.
