 ...
 * However, signed multiplication can be emulated efficiently with unsigned multiplication, and SSE
 * 4.1 is currently rare, so it is OK to not turn this on */
...
{
	// we want to compute v >> 63, that is, _mm_srai_epi64(v, 63).  But there is no 64 bit shift
	// right arithmetic instruction in SSE2.  So we have to fake it by first duplicating the high 32
	// bit values, and then using a 32 bit shift.  Another option would be to use _mm_srli_epi64(v,
	// 63) and then subtract that from 0, but that approach appears to be substantially slower for
	// unknown reasons
...
static inline __m128i libdivide_mullhi_u64_flat_vector(__m128i x, __m128i y)
{
	...
	const __m128i x0y0_hi = _mm_srli_epi64(_mm_mul_epu32(x0, y0),
	                                       32); // x0 happens to have the low half of the two 64 bit
	                                            // values in 32 bit slots 0 and 2, so _mm_mul_epu32
	                                            // computes their full product, and then we shift
...
struct libdivide_u32_t libdivide_u32_gen(uint32_t d)
{
	...
	if ((d & (d - 1)) == 0) {
		...
		if (e < (1U << floor_log_2_d)) {
			 ...
			 * However, we already have (2**(power-1))/d and its remainder.  By doubling both, and
			 * then correcting the remainder, we can compute the larger division. */
		...
		result.more = more;
		// result.more's shift should in general be ceil_log_2_d.  But if we used the smaller power,
		// we subtract one from the shift because we're using the smaller power. If we're using the
		// larger power, we subtract one from the shift because it's taken care of by the add
...
__m128i libdivide_u32_do_vector(__m128i numers, const struct libdivide_u32_t* denom)
{
	...
	if (more & LIBDIVIDE_U32_SHIFT_PATH) {
		return _mm_srli_epi32(numers, more & LIBDIVIDE_32_SHIFT_MASK);
	} else {
		const __m128i q = libdivide__mullhi_u32_flat_vector(numers, _mm_set1_epi32(denom->magic));
		if (more & LIBDIVIDE_ADD_MARKER) {
			// uint32_t t = ((numer - q) >> 1) + q;
			// return t >> denom->shift;
			const __m128i t = _mm_add_epi32(_mm_srli_epi32(_mm_sub_epi32(numers, q), 1), q);
			return _mm_srli_epi32(t, more & LIBDIVIDE_32_SHIFT_MASK);
...
struct libdivide_u64_t libdivide_u64_gen(uint64_t d)
{
	...
	if ((d & (d - 1)) == 0) {
		...
		if (e < (1ULL << floor_log_2_d)) {
			...
			/* We have to use the general 65-bit algorithm.  We need to compute (2**power) / d.
			 * However, we already have (2**(power-1))/d and its remainder.  By doubling both, and
			 * then correcting the remainder, we can compute the larger division. */
			proposed_m += proposed_m; // don't care about overflow here - in fact, we expect it
		...
		// we subtract one from the shift because we're using the smaller power. If we're using the
		// larger power, we subtract one from the shift because it's taken care of by the add
		// indicator.  So floor_log_2_d happens to be correct in both cases, which is why we do it
...
__m128i libdivide_u64_do_vector(__m128i numers, const struct libdivide_u64_t* denom)
{
	...
	if (more & LIBDIVIDE_U64_SHIFT_PATH) {
		...
		const __m128i q =
		    libdivide_mullhi_u64_flat_vector(numers, libdivide__u64_to_m128(denom->magic));
		if (more & LIBDIVIDE_ADD_MARKER) {
			// uint32_t t = ((numer - q) >> 1) + q;
			// return t >> denom->shift;
...
struct libdivide_s32_t libdivide_s32_gen(int32_t d)
{
	...
	if ((absD & (absD - 1)) ==
		...
		uint8_t more;
		// the dividend here is 2**(floor_log_2_d + 31), so the low 32 bit word is 0 and the high
		// word is floor_log_2_d - 1
		uint32_t rem, proposed_m;
		...
		if (e < (1U << floor_log_2_d)) {
			...
			/* We need to go one higher.  This should not make proposed_m overflow, but it will make
			 * it negative when interpreted as an int32_t. */
...
struct libdivide_s64_t libdivide_s64_gen(int64_t d)
{
	...
	if ((absD & (absD - 1)) ==
		...
		// the dividend here is 2**(floor_log_2_d + 63), so the low 64 bit word is 0 and the high
		// word is floor_log_2_d - 1
		uint8_t more;
...
__m128i libdivide_s64_do_vector(__m128i numers, const struct libdivide_s64_t* denom)
{
	...
	if (magic == 0) { // shift path
		...
		if (more & LIBDIVIDE_ADD_MARKER) {
			__m128i sign = _mm_set1_epi32((int32_t)((int8_t)more >> 7)); // must be arithmetic shift
			q = _mm_add_epi64(q, _mm_sub_epi64(_mm_xor_si128(numers, sign),
			                                   sign)); // q += ((numer ^ sign) - sign);
		}
...
/* The C++ template design here is a total mess.  This needs to be fixed by someone better at
templates than I.  The current design is:

- The base is a template divider_base that takes the integer type, the libdivide struct, a
generating function, a get algorithm function, a do function, and either a do vector function or a
dummy int.
- The base has storage for the libdivide struct.  This is the only storage (so the C++ class should
be no larger than the libdivide struct).

- Above that, there's divider_mid.  This is an empty struct by default, but it is specialized
against our four int types.  divider_mid contains a template struct algo, that contains a typedef
for a specialization of divider_base.  struct algo is specialized to take an "algorithm number,"
where -1 means to use the general algorithm.

- Publicly we have class divider, which inherits from divider_mid::algo.  This also take an
algorithm number, which defaults to -1 (the general algorithm).
- divider has a operator / which allows you to use a divider as the divisor in a quotient
expression.
...
/* Some bogus unswitch functions for unsigned types so the same (presumably templated) code can work
...
#if LIBDIVIDE_USE_SSE2
	/* Treats the vector as either two or four packed values (depending on the size), and divides
	 * each of them by the divisor, returning the packed quotients. */
