...
class PVPODTree : protected Alloc::template rebind<unsigned char>::other
{
  ...
  private:
	...
	struct branch_t {
		...
		inline void init_first(block_t b)
		{
			first = b;
			cur = first;
			p_cur_block = b.p;
		...
		inline T const& get_first() const
		{
			assert(valid());
			return *(first.p);
		}
	...
	class const_branch_iterator : public std::iterator<std::forward_iterator_tag, value_type>
	{
	  ...
	  protected:
		const_branch_iterator(branch_t const& branch,
		    ...
		    , _cur_index(cur_index)
		{
		}

	  public:
		const_branch_iterator() : _branch(NULL), _cur_index(0) {}
		...
		const_branch_iterator& operator++()
		{
			...
			if (_cur_index == _block_size - 1) {
				...
				_cur_index++;
				if (_cur_block.p == _branch->cur.p && _cur_index == _branch->cur_size()) {
		...
		inline const_branch_iterator& operator++(int) { return operator++(); }

	  public:
		bool operator==(const const_branch_iterator& o)
		{
			return _cur_block.p == o._cur_block.p && _cur_index == o._cur_index;
		}
		bool operator!=(const const_branch_iterator& o)
		{
			return _cur_block.p != o._cur_block.p || _cur_index != o._cur_index;
	  ...
	  private:
		inline T const& get_cur_ref() const
		{
			assert(_cur_block.p);
			return *(_cur_block.p + _cur_index);
		}
  ...
  public:
	PVPODTree() : _buf(NULL) { clear(); }

	PVPODTree(size_type nelts) : _buf(NULL)
	{
		clear();
		resize(nelts);
	}
  ...
  public:
	...
	void resize(size_type nelts)
	{
		...
		// [<-----------------------------------------> <------------------------------------->]
		// [      block 0                                   block 1                        ... ]
		// Each branch take a block from this buffer and set its pointer to the next block when
		// necessary.
	...
	bool push(size_type branch_id, T elt)
	{
		...
		} else if (cur_b.cur_size() == _nelts_block) {
	...
	{
		return ((uintptr_t)_cur_buf - (uintptr_t)_buf) /
		       (_nelts_block * sizeof(T) + sizeof(pointer));
	}
	...
	void dump_buf_stats() const
	{
		...
		PVLOG_INFO("Buffer usage: %0.4fMB/%0.4fMB (%0.4f %%)\n",
		           ((double)usage / ((1024.0 * 1024.0))), ((double)org_size) / (1024.0 * 1024.0),
		           ((double)usage / (double)org_size) * 100.0);
	}
  ...
  private:
	inline size_t size_block() const
	{
		return (size_t)_nelts_block * sizeof(value_type) + sizeof(branch_t);
	}
