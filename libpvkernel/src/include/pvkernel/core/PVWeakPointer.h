...
template <typename T> class PVWeakPtr
{
  ...
  public:
	...
	PVSharedPtr<T> lock() const { return PVSharedPtr<T>(*this); }

	long use_count() const { return _weak_count.use_count(); }

	bool expired() const { return _weak_count.use_count() == 0; }

	inline void set_deleter(typename PVSharedPtr<T>::deleter d) { lock().set_deleter(d); }

	void reset() { PVWeakPtr<T>().swap(*this); }
