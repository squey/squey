...
void launch_adaptive(F1 const& f1, F2 const& f2, TimeDuration const& dur)
{
	...
	if (!thread.timed_join(dur)) {
		// f1 takes too much time. We tell thread to stop, and launch f2. Then, waits for thread to
		// finish.
		thread.interrupt();
		thread.join();
		f2();
...
{
	boost::thread thread(
	    boost::bind(&__impl::worker_thread<F1, Tret>, boost::ref(f1), boost::ref(ret)));
	if (!thread.timed_join(dur)) {
		// f1 takes too much time. We tell thread to stop, and launch f2. Then, waits for thread to
		// finish.
		thread.interrupt();
