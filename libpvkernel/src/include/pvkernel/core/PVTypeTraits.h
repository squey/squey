...
template <class T>
inline T const* get_pointer(T const& p)
{
	return &p;
}
...
template <class T>
inline T const* get_pointer(std::shared_ptr<T const> const& p)
{
	return p.get();
}
...
typename const_fwd<
    T,
    typename std::remove_reference<typename boost::call_traits<Tref>::param_type>::type>::type&&
