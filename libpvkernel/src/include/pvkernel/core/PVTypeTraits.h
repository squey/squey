...
template <class T> struct is_shared_ptr : public std::false_type
{
};

template <class T> struct is_shared_ptr<std::shared_ptr<T>> : public std::true_type
{
};

template <class T> struct add_shared_ptr
{
	typedef std::shared_ptr<T> type;
};

template <class T> struct add_shared_ptr<std::shared_ptr<T>>
{
	typedef T type;
};

template <class T> struct remove_shared_ptr
{
	typedef T type;
};

template <class T> struct remove_shared_ptr<std::shared_ptr<T>>
{
	typedef T type;
};

// Dynamic cast T* into Y*
template <class Y, class T> struct dynamic_pointer_cast
{
	static Y cast(T p) { return dynamic_cast<T>(p); }
};

template <class Y, class T> struct dynamic_pointer_cast<std::shared_ptr<Y>, std::shared_ptr<T>>
{
...
template <class T> struct pointer<std::shared_ptr<T>>
{
	typedef std::shared_ptr<T> type;
...
template <class T> inline T const* get_pointer(T const& p)
{
	return &p;
}
...
template <class T> inline T const* get_pointer(std::shared_ptr<T const> const& p)
{
	return p.get();
}
...
namespace __impl
{
template <std::size_t mod> struct is_size_multiple_impl
{
	static const bool result = false;
};

template <> struct is_size_multiple_impl<0u>
{
	static const bool result = true;
};
}

template <class T, class MultipleOf> struct is_size_multiple
{
	static const bool value = __impl::is_size_multiple_impl<sizeof(T) % sizeof(MultipleOf)>::result;
};

template <class A, class B> struct bigger_than
{
...
template <typename T> struct add_reference_const
{
...
template <class T, class Tref> struct const_fwd
{
	typedef T type;
};

template <class T, class Tref> struct const_fwd<T, const Tref>
{
...
template <typename U, typename T,
          typename std::enable_if<std::is_polymorphic<T>::value == true, int>::type = 0>
inline typename const_fwd<typename std::remove_pointer<U>::type, T>::type*
dynamic_cast_if_possible(T* obj)
{
...
typename const_fwd<
    T, typename std::remove_reference<typename boost::call_traits<Tref>::param_type>::type>::type&&
forward_with_const(typename std::remove_reference<T>::type& t)
{
	typedef typename std::remove_reference<typename boost::call_traits<Tref>::param_type>::type
	    ref_type;
...
namespace __impl
{
template <size_t N, typename T, typename... Tparams> struct variadic_param_count_helper
{
	constexpr static size_t count = variadic_param_count_helper<N + 1, Tparams...>::count;
};

template <size_t N, typename T> struct variadic_param_count_helper<N, T>
{
	constexpr static size_t count = N + 1;
};

template <size_t I, size_t N, typename T, typename... Tparams> struct variadic_n_helper
{
	typedef typename variadic_n_helper<I, N + 1, Tparams...>::type type;
};

template <size_t I, typename T, typename... Tparams> struct variadic_n_helper<I, I, T, Tparams...>
{
	typedef T type;
};
}
 ...
 */
template <typename... Tparams> struct variadic_param_count
{
 ...
 */
template <size_t I, typename... Tparams> struct variadic_n
{
