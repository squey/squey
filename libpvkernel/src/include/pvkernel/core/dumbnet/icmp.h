...
#define ICMP_INFOTYPE(type)                                                                        \
	((type) == ICMP_ECHOREPLY || (type) == ICMP_ECHO || (type) == ICMP_RTRADVERT ||                \
	 (type) == ICMP_RTRSOLICIT || (type) == ICMP_TSTAMP || (type) == ICMP_TSTAMPREPLY ||           \
	 (type) == ICMP_INFO || (type) == ICMP_INFOREPLY || (type) == ICMP_MASK ||                     \
	 (type) == ICMP_MASKREPLY)
...
#define icmp_pack_hdr_echo(hdr, type, code, id, seq, data, len)                                    \
	do {                                                                                           \
		struct icmp_msg_echo* echo_pack_p =                                                        \
		    (struct icmp_msg_echo*)((uint8_t*)(hdr) + ICMP_HDR_LEN);                               \
...
#define icmp_pack_hdr_quote(hdr, type, code, word, pkt, len)                                       \
	do {                                                                                           \
		struct icmp_msg_quote* quote_pack_p =                                                      \
		    (struct icmp_msg_quote*)((uint8_t*)(hdr) + ICMP_HDR_LEN);                              \
		icmp_pack_hdr(hdr, type, code);                                                            \
...
#define icmp_pack_hdr_mask(hdr, type, code, id, seq, mask)                                         \
	do {                                                                                           \
		struct icmp_msg_mask* mask_pack_p =                                                        \
		    (struct icmp_msg_mask*)((uint8_t*)(hdr) + ICMP_HDR_LEN);                               \
		icmp_pack_hdr(hdr, type, code);                                                            \
