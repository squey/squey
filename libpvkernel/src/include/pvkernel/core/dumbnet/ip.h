...
#define ip_pack_hdr(hdr, tos, len, id, off, ttl, p, src, dst)                                      \
	do {                                                                                           \
		...
		ip_pack_p->ip_v = 4;                                                                       \
		ip_pack_p->ip_hl = 5;                                                                      \
		ip_pack_p->ip_tos = tos;                                                                   \
		ip_pack_p->ip_len = htons(len);                                                            \
		ip_pack_p->ip_id = htons(id);                                                              \
		ip_pack_p->ip_off = htons(off);                                                            \
		ip_pack_p->ip_ttl = ttl;                                                                   \
		ip_pack_p->ip_p = p;                                                                       \
		ip_pack_p->ip_src = src;                                                                   \
		ip_pack_p->ip_dst = dst;                                                                   \
	} while (0)
