...
class PVMatrixMemory
{
  ...
  public:
	PVMatrixMemory(allocator_type const& alloc) : _alloc(alloc) {}
...
class PVMatrixMemory<T, PVMatrixAllocatorMmap>
{
  ...
  public:
	...
	inline pointer reallocate(pointer p, size_t old_n, size_t n)
	{
		...
		// (double)n*(double)sizeof(value_type)/((double)1024*1024));
		return (pointer)mremap(p, sizeof(value_type) * old_n, sizeof(value_type) * n,
		                       MREMAP_MAYMOVE);
	}
...
class PVMatrix
{
  ...
  public:
	class PVMatrixLine
	{
	  ...
	  public:
		...
		inline void set_value(index_col col, const_reference v)
		{
			assert(col < _size);
			_p[col] = v;
		}
	...
	class PVMatrixColumn
	{
	  ...
	  public:
		...
		inline void set_value(index_row row, const_reference v)
		{
			assert(row < _size);
			_p[offset_for_row(row)] = v;
		}
  ...
  public:
	PVMatrix(allocator_type const& a = allocator_type())
	    : _ncols(0), _nrows(0), _nrows_physical(0), _data(NULL), _alloc(a), _own_data(false)
	{
	}
  ...
  public:
	...
	{
		__impl::PVMatrixComputation<value_type, index_row, index_col>::set_rows_value(_data, a, b,
		                                                                              _ncols, v);
	}
	...
	inline bool reserve(index_row nrows, index_col ncols) { return _allocate(nrows, ncols); }
  ...
  public:
	...
	inline line get_row(index_row row)
	{
		assert(_data);
		assert(row < _nrows);
		return line(*this, row);
	}
	inline const_line get_row(index_row row) const
	{
		assert(_data);
		assert(row < _nrows);
		return const_line(*this, row);
	}

	inline line operator[](index_row row) { return get_row(row); }
	inline const_line operator[](index_row row) const { return get_row(row); }

	inline column get_col(index_col col)
	{
		assert(_data);
		assert(col < _ncols);
		return column(*this, col);
	}
	inline const_column get_col(index_col col) const
	{
		assert(_data);
		assert(col < _ncols);
		return const_column(*this, col);
	}

	inline pointer get_row_ptr(index_row row)
	{
		assert(_data);
		assert(row < _nrows);
		return &_data[row * _ncols];
	}
	inline const_pointer get_row_ptr(index_row row) const
	{
		assert(_data);
		assert(row < _nrows);
		return &_data[row * _ncols];
	...
	inline reference at(index_row row, index_col col)
	{
		assert(row < _nrows);
		assert(col < _ncols);
		return _data[row * _ncols + col];
	}
	inline const_reference at(index_row row, index_col col) const
	{
		assert(row < _nrows);
		assert(col < _ncols);
		return _data[row * _ncols + col];
	...
	void set_value(index_row row, index_col col, const_reference v)
	{
		assert(row < _nrows);
		assert(col < _ncols);
		_data[row * _ncols + col] = v;
	}
  ...
  private:
	...
	{
		__impl::PVMatrixComputation<value_type, index_row, index_col>::transpose(res, _data, _nrows,
		                                                                         _ncols);
	}
...
#pragma omp parallel for
	...
	inline bool _allocate(index_row nrows, index_col ncols)
	{
		...
		try {
			...
			PVLOG_ERROR("(PVMatrix::_allocate) unable to allocate %ld x %ld (%ld bytes).", nrows,
			            ncols, nrows * ncols * sizeof(value_type));
			return false;
		}
		_own_data = true;
		_data = p;
