                     ...
                     RandomAccessIterator2 result, Distance step_size, Compare comp)
{
	...
	while (last - first >= two_step) {
		result =
		    move_merge(first, first + step_size, first + step_size, first + two_step, result, comp);
		first += two_step;
	}
                    ...
                    Distance buffersize, Compare comp, tbb::task_group_context* context)
{
	if (len1 <= len2 && len1 <= buffersize) {
		...
		BidirectionalIterator newmiddle =
		    rotate_adaptive(firstcut, middle, secondcut, len1 - len11, len22, buffer, buffersize);
		/*tbb::parallel_invoke([=]{merge_adaptive(first, firstcut, newmiddle, len11, len22, buffer,
		   buffersize, comp, context);},
		                     [=]{merge_adaptive(newmiddle, secondcut, last, len1 - len11, len2 -
		   len22, buffer, buffersize, comp, context);}, *context);*/
		merge_adaptive(first, firstcut, newmiddle, len11, len22, buffer, buffersize, comp, context);
		merge_adaptive(newmiddle, secondcut, last, len1 - len11, len2 - len22, buffer, buffersize,
		               comp, context);
	}
}
                          ...
                          Distance buffer_size, Compare comp, tbb::task_group_context* context)
{
	...
	if (len > buffer_size) {
		// tbb::parallel_invoke([=]{stable_sort_adaptive(begin, middle, buffer, buffer_mid_size,
		// comp, context);},
		//                     [=]{stable_sort_adaptive(middle, end, buffer + buffer_mid_size,
		//                     buffer_mid_size, comp, context);}, *context);
	...
	merge_adaptive(begin, middle, end, Distance(middle - begin), Distance(end - middle), buffer,
	               buffer_size, comp, context);
}
                         ...
                         tbb::task_group_context* context)
{
	...
	merge_without_buffer(first, middle, last, middle - first, last - middle, comp);
}
}
