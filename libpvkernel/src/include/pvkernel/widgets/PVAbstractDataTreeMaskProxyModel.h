...
class PVAbstractDataTreeMaskProxyModel : public QAbstractProxyModel
{
  ...
  public:
	PVAbstractDataTreeMaskProxyModel(QObject* parent = nullptr) : QAbstractProxyModel(parent) {}
	...
	virtual void setSourceModel(QAbstractItemModel* src_model)
	{
		...
		if (old_src_model) {
			disconnect(old_src_model, SIGNAL(dataChanged(const QModelIndex&, const QModelIndex&)),
			           this, SLOT(data_changed(const QModelIndex&, const QModelIndex&)));

			disconnect(old_src_model, SIGNAL(modelAboutToBeReset()), this,
		...
		if (src_model) {
			...
			connect(src_model, SIGNAL(dataChanged(const QModelIndex&, const QModelIndex&)), this,
			        SLOT(data_changed(const QModelIndex&, const QModelIndex&)));
			connect(src_model, SIGNAL(modelAboutToBeReset()), this,
			        SLOT(model_about_to_be_reset()));
		}
