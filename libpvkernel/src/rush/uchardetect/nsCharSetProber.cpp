                                                    ...
                                                    PRUint32& newLen)
{
	...
	for (curPtr = prevPtr = (char*)aBuf; curPtr < aBuf + aLen; curPtr++) {
		if (*curPtr & 0x80) {
			...
			// this segment contains more than single symbol, and it has upper ASCII, we need to
			// keep it
			{
				...
				*newptr++ = ' ';
				meetMSB = PR_FALSE;
			} else // ignore current segment. (either because it is just a symbol or just an English
				   // word)
				prevPtr = curPtr + 1;
		}
	}
