...
{
	// Keep a copy of the DOM document if the format has been automatically
	// generated by a connector and is not stored on disk.
	// This will allow to serialize it later.
	if (_full_path.isEmpty()) {
...
void PVRush::PVFormat::serialize_write(PVCore::PVSerializeObject& so) const
{
	if (not _full_path.isEmpty()) {
	...
	} else { // This format does not exist on disk : use the stored DOM
		...
		_dom.save(stream, 4 /*indent*/, QDomNode::EncodingFromTextStream);
