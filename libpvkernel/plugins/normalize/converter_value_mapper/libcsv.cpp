...
csv_init(struct csv_parser *p, unsigned char options)
{
  ...
  p->status = 0;
  p->current_line = 0;
  p->limit_min = 0;		/* Line we start reading */
  p->limit_max = 0;		/* Line we stop reading */
  p->options = options;
...
void
csv_set_limit_max(struct csv_parser *p, long limit)
{
  if (p) p->limit_max = limit;
}
...
{
  const unsigned char *us = (const unsigned char *)s;  /* Access input data as array of unsigned char */
