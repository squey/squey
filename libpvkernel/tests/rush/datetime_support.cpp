...
int main()
{
	...
	//testcases.emplace_back("datetime_ms", "yyyy-M-dH:m:s.SZ", 		"2015-3-2700:00:07.1882+01:00");         // bug in ICU : not deterministic
	//testcases.emplace_back("datetime_ms", "eee MMM d H:m:s V yyyy", 	"Tue Nov 9 13:11:46 EST 2010");          // bug in ICU : wrong year

	for (const testcase_t& testcase : testcases) {

		const pvcop::formatter_desc& fd = PVRush::PVFormat::get_datetime_formatter_desc(testcase.time_format);

		pvcop::types::formatter_interface* fi = pvcop::types::factory::create(fd.name(), fd.parameters());
		...
		if (fi->from_string(input_string, out_array.data(), 0)) { //from string
			...
			if(fi->to_string(converted_string, str_len, out_array.data(), 0) > 0) { //to string
				if(strcmp(converted_string, output_string) != 0) {
					...
					std::cerr << "date: '" << output_string << "' - datetime format: '" << testcase.time_format
										<< "' - formatter parameters: '" << fd.parameters() << "' - pvcop generate number: " << out_array.at(0)
				...
				std::cerr << " date : " << output_string << " - datetime format: " << testcase.time_format
					<< " - formatter parameters : " << fd.parameters() << " - pvcop underlying value : " << out_array.at(0)
					<< " - pvcop generated string: " << converted_string << std::endl;
				return 1;
			}
			...
			std::cerr << " date: " << input_string << " - datetime format: " << testcase.time_format
				<< " - formatter parameters: " << fd.parameters() << " - pvcop underlying value: " << out_array.at(0) << std::endl;
			return 1;
		}
	}
