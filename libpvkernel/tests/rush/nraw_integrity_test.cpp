...
constexpr uint32_t COLUMN_COUNT = 8;
constexpr uint32_t ROW_COUNT = 200000;
...
int main(int argc, char** argv)
{
	...
	ranges.push_back(abstract_generator_sp(new fixed_generator(11)));
	ranges.push_back(abstract_generator_sp(new fixed_generator(15)));
	ranges.push_back(abstract_generator_sp(new fixed_generator(16)));
	ranges.push_back(abstract_generator_sp(new random_gaussian_generator(6, 16, 0.3, 0.1)));
	ranges.push_back(abstract_generator_sp(new random_generator(10, 15)));
	...
	for (uint32_t t = 0; t < ranges.size(); t++) {
		...
		std::vector<PVRush::PVNrawDiskBackend::unique_values_t> random_strings_unique;
		random_strings_vect.resize(COLUMN_COUNT);
		random_strings_unique.resize(COLUMN_COUNT);
		for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
			...
			for (uint32_t r = 0 ; r < ROW_COUNT; r++) {
				...
				random_strings_unique[c].insert(std::string_tbb(rs.c_str(), rs.size()));
			}
		}
		backend.flush();
		...
		for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
			...
			{
				ASSERT_VALID(random_strings_vect[c][index].compare(0, size1, buf1) == 0);
			});
		}
		...
		for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
			backend.visit_column2_sel(c, [&](size_t index, const char* buf1, size_t size1)
			{
				ASSERT_VALID(vec_sel[index] == true);
				ASSERT_VALID(random_strings_vect[c][index].compare(0, size1, buf1) == 0);
			},
			sel);
		}
		...
		*/
		...
		PVLOG_INFO("Verify nraw integrity using random selected lines 'visit_column_tbb_sel'... ");
		for (size_t i = 0; i < 10; i++) {
			std::cout << "Random selection " << i << "..." << std::endl;
			BENCH_START(visit_column_tbb_sel_rand);
			sel.select_random();
			uint32_t nelts_sel = sel.get_number_of_selected_lines_in_range(0, ROW_COUNT);
			tbb::atomic<uint32_t> nelts_found;
			for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
				nelts_found = 0;
				backend.visit_column_tbb_sel(c, [&](size_t index, const char* buf1, size_t size1)
				{
					nelts_found++;
					ASSERT_VALID(index < ROW_COUNT);
					ASSERT_VALID(random_strings_vect[c][index].compare(0, size1, buf1) == 0);
				},
				sel);
				ASSERT_VALID(nelts_found == nelts_sel);
			}
			std::cout << "test_passed" << std::endl;
			BENCH_END(visit_column_tbb_sel_rand, "visit_column_tbb_sel_rand", 1, 1, 1, 1);
		}

		PVLOG_INFO("Verify sort/uniq integrity using all selected lines... ");
		BENCH_START(sort_uniq_all_sel);
		sel.select_all();
		for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
			PVRush::PVNrawDiskBackend::unique_values_t uniqv;
			backend.get_unique_values_for_col_with_sel(c, uniqv, sel);
			ASSERT_VALID(uniqv == random_strings_unique[c]);
		}
		std::cout << "test_passed" << std::endl;
		BENCH_END(sort_uniq_all_sel, "sort_uniq_all_sel", 1, 1, 1, 1);

		PVLOG_INFO("Verify nraw integrity using random selected lines 'visit_column_tbb_sel'... ");
		for (size_t i = 0; i < 10; i++) {
			std::cout << "Random selection " << i << "..." << std::endl;
			BENCH_START(sort_uniq_rand_sel);
			sel.select_random();
			for (uint32_t c = 0 ; c < COLUMN_COUNT; c++) {
				PVRush::PVNrawDiskBackend::unique_values_t uniqv;
				backend.get_unique_values_for_col_with_sel(c, uniqv, sel);
				PVRush::PVNrawDiskBackend::unique_values_t uniqv_ref;
				sel.visit_selected_lines([&](const PVRow r)
					{
						std::string const& str = random_strings_vect[c][r];
						uniqv_ref.insert(std::string_tbb(str.c_str(), str.length()));
					}, ROW_COUNT);
				ASSERT_VALID(uniqv == uniqv_ref);
			}
			BENCH_END(sort_uniq_rand_sel, "sort_uniq_rand_sel", 1, 1, 1, 1);
		}
	...
	#pragma omp parallel for
			for (uint32_t r = 0 ; r < ROW_COUNT; r++) {
				...
				ASSERT_VALID(random_strings_vect[c][index].compare(0, size, buf1) == 0);
			}
		}
		std::cout << "test_passed" << std::endl;
