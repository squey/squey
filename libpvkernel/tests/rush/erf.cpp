...
{
	if (argc <= 3) {
		std::cerr << "Usage: " << argv[0] << " <erf_file> <json_selected_nodes> <output_ref1>  <output_ref2>  <output_ref...>" << std::endl;
		return 1;
	}
	...
	std::vector<QDomDocument> formats = erf.get_formats_from_selected_nodes(selected_nodes);

	QList<std::shared_ptr<PVRush::PVInputDescription>> list_inputs;
	PVRush::PVERFDescription* erf_desc = new PVRush::PVERFDescription(QString::fromStdString(erf_file), std::move(selected_nodes));
	list_inputs << PVRush::PVInputDescription_p(erf_desc);

	for (size_t i = 0; i < formats.size(); i++) {
		const PVRush::PVFormat& format = PVRush::PVFormat(formats[i].documentElement());
		PVRush::PVNraw nraw;
		...
		std::cout << std::endl << output_file << " - " << output_refs[i] << std::endl;
		PV_ASSERT_VALID(PVRush::PVUtils::files_have_same_content(output_file, output_refs[i]));
	#endif
