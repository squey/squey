...
int main()
{
	...
	std::cout << "test1_p=" << test1_p1
	          << ", test1_p->shared_from_this().get()=" << test1_p1->shared_from_this().get()
	          << std::endl;
	PV_VALID(test1_p1->shared_from_this().use_count(), 2L);
	std::cout << "test_p1->shared_from_this().use_count()="
	          << test1_p1->shared_from_this().use_count() << std::endl;
	...
	// PVWeakPtr count is working properly (increment, decrement)
	{
		...
		PV_ASSERT_VALID((test1_wp1.use_count() == test1_sp2.use_count()) &&
		                (test1_sp2.use_count() == test1_sp3.use_count()) &&
		                (test1_sp3.use_count() == 3));
		std::cout << "test1_wp1.use_count()=" << test1_wp1.use_count() << std::endl;
	}
	PV_VALID(test1_wp1.use_count(), 1L);
	std::cout << "test1_wp1.use_count()=" << test1_wp1.use_count() << std::endl;
	...
	// test1_sp1.reset(test1_p3, &deleter<Test1>) correctly reset the pointer, the deleter and the
	// count
	...
	std::cout << "test2_p1=" << test2_p1
	          << ", test2_p1->shared_from_this().get()=" << test2_p1->shared_from_this().get()
	          << std::endl;
	PV_VALID(test2_p1->shared_from_this().use_count(), 2L);
	std::cout << "test2_p1->shared_from_this().use_count()="
	          << test2_p1->shared_from_this().use_count() << std::endl;
	...
	std::cout << "test1_sp4.get()=" << test1_sp4.get() << ", test2_sp2.get()=" << test2_sp2.get()
	          << ", test2_sp3.get()=" << test2_sp3.get() << std::endl;

	// Test PVSharedPtr creation from a pointer of a derived type
	Test2* test2_p4 = new Test2();
	Test1_sp test1_sp5(test2_p4); // Warning: unlikely to std::shared_ptr, the derived destructor
	                              // has to be virtual to be called.
